...
const ProjectsSearchComponent = () => {
  ...
  const projectsFiltrados = Array.isArray(projects) ? projects.filter(project => 
    project.title && project.title.toLowerCase().includes(busqueda.toLowerCase())
  ) : [];
  // Crear proyecto
  ...
  const handleEditDialogOpen = (projectId, projectData) => {
    ...
    const userNames = projectData.users?.map(user => {
      return user ? `${user.firstName} ${user.lastName}` : null;
    }).filter(userName => userName !== null);

    const listNames = projectData.lists?.map(listId => {
  ...
  return (
    <>
        <div style={{paddingBlock: '1rem', display: 'flex'}} className='contentSearch'>
            ...
            />
            <Button style={{marginLeft: 'auto'}} variant="contained" color="primary" onClick={handleAddDialogOpen}>Añadir proyecto</Button>
        </div>
        <Dialog open={openDialog} onClose={handleDialogClose}>
          <DialogTitle>{editMode ? 'Editar proyecto' : 'Añadir nuevo proyecto'}</DialogTitle>
          <DialogContent>
            <TextField autoFocus margin="dense" name="title" label="Título" type="text" fullWidth variant="standard" value={newProject.title || ''} onChange={handleChange} />
            <TextField margin="dense" name="description" label="Descripción" type="text" fullWidth variant="standard" value={newProject.description || ''} onChange={handleChange} />
          ...
          <DialogActions>
            ...
            <Button onClick={() => { saveProject(newProject); handleDialogClose(); }}>{editMode ? 'Guardar' : 'Añadir'}</Button>
          </DialogActions>
        </Dialog>
            ...
            <TableBody>
              {projectsFiltrados.map((project, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    {project.lists && project.lists.length > 0 ? project.lists.map(list => list.name).join(', ') : "No hay listas asignadas"}
                  </TableCell>
                  <TableCell>
                    {project.users.map(user => `${user.firstName} ${user.lastName}`).join(', ')}
                  </TableCell>
                  <TableCell>
                    <Button variant="contained" color="primary" onClick={() => handleEditDialogOpen(project._id, project)}><EditIcon /></Button>
                  </TableCell>
                  <TableCell>
                    <Button variant="contained" color="secondary" onClick={() => deleteProject(project._id)}><DeleteIcon /></Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
