  ...
  return (
    <>
        <div style={{paddingBlock: '1rem', display: 'flex'}} className='contentSearch'>
            ...
            />
            <Button style={{marginLeft: 'auto'}} variant="contained" color="primary" onClick={handleAddDialogOpen}>Añadir lista</Button>
        </div>
        <Dialog open={openDialog} onClose={handleDialogClose}>
          <DialogTitle>{editMode ? 'Editar lista' : 'Añadir nueva lista'}</DialogTitle>
          <DialogContent>
            <TextField autoFocus margin="dense" name="name" label="Nombre" type="text" fullWidth variant="standard" value={newList.name || ''} onChange={handleChange} />
          </DialogContent>
          <DialogActions>
            <Button onClick={handleDialogClose}>Cancelar</Button>
            <Button onClick={() => { saveList(newList); handleDialogClose(); }}>{editMode ? 'Guardar' : 'Añadir'}</Button>
          </DialogActions>
        </Dialog>
            ...
            <TableBody>
              {listsFiltrados.map((list, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <Button variant="contained" color="primary" onClick={() => handleEditDialogOpen(list._id, list)}><EditIcon /></Button>
                  </TableCell>
                  <TableCell>
                    <Button variant="contained" color="secondary" onClick={() => deleteList(list._id)}><DeleteIcon /></Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
