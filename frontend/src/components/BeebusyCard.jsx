...
import { Paper, IconButton, TextField, Dialog, DialogTitle, DialogContent, DialogActions, Button } from "@mui/material";
...
const BeebusyCard = ({ projectId, taskid, content, onUpdateTask }) => {
    ...
    const saveTask = (updatedTask) => {
      ...
      const userIds = projectUsers.filter(user => updatedTask.users.includes(`${user.firstName} ${user.lastName}`)).map(user => user._id);
      ...
      .then(data => {
        ...
        const taskData = tasks.find(task => task._id === taskid);
        if (taskData) {
            const updatedUsers = taskData.users.map(user => `${user.firstName} ${user.lastName}`);
        ...
        <>
            <Dialog open={openDialog} onClose={handleDialogClose}>
                ...
                <DialogContent>
                    ...
                    <TextField margin="dense" name="startDate" label="Fecha de inicio" type="date" fullWidth value={moment(newTask.startDate).format('YYYY-MM-DD')} onChange={handleChange} />
                    <TextField margin="dense" name="dueDate" label="Fecha de finalización" type="date" fullWidth value={moment(newTask.dueDate).format('YYYY-MM-DD')} onChange={handleChange} />
                </DialogContent>
                <DialogActions>
                    <Button onClick={handleDialogClose} color="primary">Cancelar</Button>
                    <Button onClick={() => { saveTask(newTask); handleDialogClose(); }} color="primary">Guardar</Button>
                </DialogActions>
            </Dialog>
